...
from prediction import Prediction, VesselTree
...
def trace_centerline(output_folder, image_file, case, model_folder, modality, img_shape, threshold, stepsize, vessel_tree, seg_file=None, write_samples=True):
    ...
    for i in range(500):
        ...
        try:
            ...
            step = vessel_tree.steps[i]

            polydata_point = vf.points2polydata([step['point'].tolist()])
            ...
            # Extract Volume
            volume_size = 5
            size_extract, index_extract = sf.map_to_image(step['point'], step['radius'], volume_size, origin_im, spacing_im)
            step['img_index'] = index_extract
            step['img_size'] = size_extract
            ...
            step['point_pd'] = polydata_point
            step['surface'] = surface_smooth
            step['centerline'] = centerline_poly

            vessel_tree.add_step(i, step, branch)
            ...
            if point_tree.size != 0:
                next_step = {}
                next_step['old_point'] = point
                next_step['old_radius'] = radius
                next_step['angle'] = angle_change[0]
                next_step['point'] = point_tree[0]
                next_step['radius'] = radius_tree[0]*1.1
...
if __name__=='__main__':
    ...
    init_step = {}
    init_step['old point'] = old_seed
    init_step['point'] = initial_seed
    init_step['old radius'] = old_radius
    init_step['radius'] = inital_radius
    init_step['angle change'] = 0
    vessel_true = VesselTree(case, dir_image, init_step)
    ## Trace centerline
    centerlines, surfaces, points, assembly, vessel_tree = trace_centerline(dir_output, dir_image, case, dir_model_weights, modality, nn_input_shape, threshold, stepsize, vasculature, dir_seg, write_samples=True)
